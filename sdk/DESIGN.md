...
**① 跨进程并发未命中 → 重复条目。** 合流是进程内的。两个进程同时未命中同一个
问题，会各自生成、各自写入，产生两条同 `(scope, matchHash)` 的条目。

- 后果是**浪费，不是错误**：多付一次生成、多一行存储。
...
### 没有做的：`(scope, match_hash)` 唯一约束

看起来是根治方案：唯一约束 + upsert，重复条目从源头消失。**没做，因为
`matchHash` 是非密码学哈希。** 加上唯一约束等于让一次哈希碰撞变成「两个完全不同的
问题互相覆盖」—— 而 ② 这一层的全部价值就是零假命中风险，它现在的做法正是
「哈希命中之后再比一次原文」。

要做的话得另加一列强哈希（归一化文本的 SHA-256）作唯一键，那是一次 schema 变更
加一次 ② 的语义变更，值得单独想清楚。
