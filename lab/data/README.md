...
`langcache-*` 和 `qqp.json` 只有句对，**没有资料、没有资料版本**，所以只够测 ②③④ 的
问题侧判断。⑤ 版本比对要「答案 ↔ 资料片段」，它的标定仍然留在课程语料上
（`scripts/calibrate.ts`）。④ 的**问↔答**形态同样需要答案文本，QQP 也给不了。

`vcache-*` 补上了答案和等价组，但**没补上资料版本** —— 那四份基准里没有「同一个问题、
资料改版后答案变了」这种条目。⑤ 至今没有一份公开数据可用，候选（FreshQA、RealTimeQA、
StreamingQA、Time-Sensitive-QA）都还没接。
