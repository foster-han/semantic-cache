...
分层语义缓存：把「问的是不是同一件事」和「那个旧答案现在还立得住吗」拆成五道各管一类
失效的闸，而不是一个相似度阈值一刀切。阈值不许借别人的 —— 没有标注数据时，
从语料自己生成探针来标。
...
单测覆盖的是那些**出错也不报错**的不变式：票据配错 prompt、没有依据的答案不写入、
③ 拿回候选后要复核 scope、替换必须先写后删、判别力判据是 margin 而不是跨度、
标不出阈值时拿不到能用的 `calibratedOn`。每一条都对应
[`FINDINGS.md`](FINDINGS.md#踩过的坑) 里的一个坑。
